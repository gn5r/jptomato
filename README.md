...
`sudo apt update && sudo apt install -y git`<br>
Gitの最新パッケージをインストールする為、aptを更新してからインストール。
...
これら3つのコマンドを打ち込んでおいてください。<br>
**--global**オプションを指定した場合、全てのディレクトリで適用されるので、ディレクトリごとにアカウントを変えたい等があるならば、<br>
**--global**オプションを外しても構いません。<br>
また、3つ目のコマンドはパスワードを保存する為のものです。**cache**にすると一時記憶され15分の間はユーザー名・パスワードの入力なしでpush出来ます。
...
以下画像はREADMEファイルの編集中の画像です。

![画像5](./img/code.png)

Cmderでローカルリポジトリをカレントディレクトリとして開き、`git status`と打ち込んでください。赤い文字で何やら出てくると思います。<br>

![画像6](./img/cmder_modify.png)

このような画面が出てくると思います。何かファイルに対し変更があると、赤文字で知らせてくれます。<br>
ではこの変更をGitHubのwebページで作成したリモートリポジトリに反映させましょう。<br>
...
`git add ファイル名(拡張子も記入)`または`git add .(ドット)`<br>
...
<a name="command"></a>
## コマンドリスト集
Gitコマンドとよく使うShellコマンドのリストを作ってあるので、そちらも参考にしてみて下さい<br>
Cmderでも使えるコマンドなのでLinuxじゃなきゃ出来ない！という訳ではないので安心して下さい
